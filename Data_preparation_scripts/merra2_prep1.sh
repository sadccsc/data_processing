...
for institute in ECMWF ; 
	...
	for dataset in  ERA5;
		do
		path_observations_native=$( echo  ../data/reanalysis/${domain}/gridded/${institute}/${dataset}/${period}/${region} )
		...
		for files in $path_observations_native/pr_*merged.nc ;
			do
		 	...
		 	cp   $path_observations_native/`basename $files .nc`_mm.nc   $path_observations_regridded/pr_day_${institute}_${dataset}_merged_lonlat_era5.nc 
	 	done 
