...
domain=$( echo 'global' )
region=$( echo 'sadc')
for institute in GPM ; 
	do
	for dataset in IMERG;
		do
		path_observations_native=$( echo /Volumes/tiro/PhD/data/Gridded_Observations/pr/GPM/ )
		...
		for files in $path_observations_native/*SA_1.nc ;
			do
			#cdo chname,precipitationCal,pr $path_observations_native/`basename $files .nc`.nc $path_observations_native/`basename $files .nc`_pr_SA.nc
			#cdo remapbil,'era5_grid' $path_observations_native/`basename $files .nc`_pr_SA.nc $path_observations_regridded/pr_day_${institute}_${dataset}_merged_lonlat_era5.nc
